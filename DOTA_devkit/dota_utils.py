...
def get_best_begin_point(coordinate):
    ...
    force_flag = 0
    for i in range(4):
        temp_force = cal_line_length(combined[i][0], dst_coordinate[0]) + cal_line_length(combined[i][1],
                                                                                           dst_coordinate[
                                                                                               1]) + cal_line_length(
            combined[i][2], dst_coordinate[2]) + cal_line_length(combined[i][3], dst_coordinate[3])
        if temp_force < force:
            force = temp_force
            force_flag = i
    if force_flag != 0:
        print("choose one direction!")
    return combined[force_flag]
